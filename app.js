...
(function () {
  ...
  function applyFiles(files) { state.catalog = window.JOEParse.catalog(files); render(); }
  function utcStamp() {
    var d = new Date();
  ...
  async function readFS() {
    ...
    for await (var entry of state.dirHandle.values()) {
      if (entry.kind === "file") {
        if (window.JOEParse.isIgnoredName(entry.name)) continue;
        ...
        for await (var mod of entry.values()) {
          if (mod.kind !== "file" || window.JOEParse.isIgnoredName(mod.name)) continue;
  ...
  async function pickDriveFolder() {
    var q = "(mimeType = 'application/vnd.google-apps.folder') and (name = 'Joe_OS' or name = 'Joe_OS_2') and trashed = false";
    var data = await driveGet("https://www.googleapis.com/drive/v3/files?pageSize=25&fields=files(id,name)&q=" + encodeURIComponent(q));
    var found = data.files || [];
    var live = [];
    for (var i = 0; i < found.length; i++) {
      var names = (await listChildren(found[i].id)).map(function (k) { return k.name; });
      if (window.JOEParse.workshopHalt(names)) continue;
      live.push(found[i]);
    }
  ...
  async function readDrive() {
    ...
    for (var i = 0; i < kids.length; i++) {
      ...
      if (window.JOEParse.isIgnoredName(k.name)) continue;
    ...
    if (modulesId) {
      ...
      for (var m = 0; m < mods.length; m++) {
        if (window.JOEParse.isIgnoredName(mods[m].name) || !/\.md$/i.test(mods[m].name)) continue;
  ...
  function init() {
    ...
    else if (/[?&]react=1/.test(location.search)) react();
    else render();
